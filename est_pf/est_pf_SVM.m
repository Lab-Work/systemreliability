...
%estimate network pf based on both the SVM and the dfs method using a
%set of damaged test networks.
clear all
close all
tic

load ../model_building/fitted_model_SVM.mat
...
PGV = attenuation(Mj, H, scaledDistToEpicenter); %compute the peak ground velocity
...
%%%% Turn on or off sigma = rho_YY to turn on or off correlation
sigma = rho_YY;

%build a test set using the number of test points specified with the
%correct component failure probabilities
link_status=build_test_set(mu', sigma, num_points, pf);

[ml_pred, dfs_pred, ml_time, dfs_time]= ...
    det_network_status_SVM(num_points, link_status, SVMModel, networkStruct);

disp(strcat('The cumulative time to complete all calculations using DFS is: ', num2str(dfs_time), 's'))
disp(strcat('The cumulative time to complete all calculations using the ML model is: ', num2str(ml_time), 's'))

[avg_dfs, avg_ml, cov_dfs, cov_ml]=...
    compute_avg(num_points, dfs_pred, ml_pred);
bias=abs(avg_ml(end)-avg_dfs(end)); %compute bias
disp(strcat('The bias is: ', num2str(bias)))
C = confusionmat(dfs_pred, ml_pred); %construct the confusion matrix
...
FNset = link_status(:,dfs_pred==1 & ml_pred==0); %false negative set
FPset = link_status(:,dfs_pred==0 & ml_pred==1); %false positive set
